...
const getAllReservations = async (req, res) => {
  try {
    ...
    const reservationFilter = {
      ...
      ...(searchQuery && { vehicleId: { $in: vehicleIds } }),
    };
    ...
    const enrichedReservations = await Promise.all(
      reservations.map(async (reservation) => {
        ...
        if (reservation.vehicleId) {
          vehicleDetails = await NewVehicle.findOne({
...
const getAllReservationsFromPanel = async (req, res) => {
  try {
    const reservations = await Reserve.find({
      $and: [{ reservation: true }, { booking: false }, { fromAdmin: true }],
    });
...
const acceptReservation = async (req, res) => {
  try {
    ...
    if (!payment) {
      return res
        .status(404)
        .json({ message: "Payment record not found for this reservation." });
    }
...
const getReservationListingByDriverID = async (req, res) => {
  try {
    ...
    if (reservations.length === 0) {
      return res
        .status(404)
        .json({ message: "No reservations found for this driver" });
    }
